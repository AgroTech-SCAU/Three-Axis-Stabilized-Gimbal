...
#define ARM_REVERSED_SERVO_ID 2u
...
static float reversed_servo_logical_to_physical(float logical_position);
static float reversed_servo_physical_to_logical(float physical_position);
static BusServoStatus arm_servo_batch_set_pos_spd(const uint8_t* ids,
                                                  const float* positions,
                                                  uint8_t count,
                                                  float velocity);
static BusServoStatus arm_servo_batch_update_feedback(const uint8_t* ids,
                                                      uint8_t count,
...
SystemStatus assemble_arm(void) {
...
#if ASSEMBLE_ARM_ENABLE_MECHANICAL_ARM
    /* UART7 当前由视觉模块用 1-byte IT 接收占用；禁止再混用阻塞舵机协议，
       否则 HAL UART 状态机会互相打断。若要恢复真实机械臂，请先在硬件/CubeMX 中为其改串口。 */
    if(uart_rx_complete_callback_registered(&huart7)) {
        log_error("ARM UART7 conflict: vision interrupt RX already owns UART7");
    ...
    arm_config.batch_set_pos_spd = arm_servo_batch_set_pos_spd;
    arm_config.batch_update_feedback = arm_servo_batch_update_feedback;
#else
...
/*
 * ID2 is mounted in reverse.  The arm model keeps the same logical angle as
 * the correctly mounted robot (90 -> 180 -> 270 degrees), while the physical
 * servo travels 90 -> 0/360 -> 270 degrees.
 */
static float reversed_servo_logical_to_physical(float logical_position) {
    /* ID2 uses ST3215 multi-turn position: 90 -> 0 -> -90 degrees. */
    return M_PI - logical_position;
}

static float reversed_servo_physical_to_logical(float physical_position) {
    const float two_pi = 2.0f * M_PI;

    while(physical_position < 0.0f) {
        physical_position += two_pi;
    }
    while(physical_position >= two_pi) {
        physical_position -= two_pi;
    }

    /* 0 degrees is the vertical point (logical 180 degrees). */
    if(physical_position <= M_PI * 0.5f) {
        return M_PI - physical_position;
    }
    return 3.0f * M_PI - physical_position;
}
...
static BusServoStatus arm_servo_batch_set_pos_spd(const uint8_t* ids,
                                                  ...
                                                  uint8_t count,
                                                  float velocity) {
    float physical_positions[ARM_DOF];
    ...
    for(uint8_t i = 0u; i < count; i++) {
        physical_positions[i] = (ids[i] == ARM_REVERSED_SERVO_ID)
                                    ? reversed_servo_logical_to_physical(positions[i])
                                    : positions[i];
        if(ids[i] == ARM_REVERSED_SERVO_ID) {
            log_info("ARM ID2 TX logical=%.4f rad (%.1f deg), physical=%.4f rad (%.1f deg), speed=%.4f rad/s",
                     positions[i],
                     RAD_TO_DEG(positions[i]),
                     physical_positions[i],
                     RAD_TO_DEG(physical_positions[i]),
                     velocity);
        }
    }
    return ft_scs_sync_write_pos_spd(ids, physical_positions, count, velocity);
}
...
static BusServoStatus arm_servo_batch_update_feedback(const uint8_t* ids,
    ...
    BusServoStatus ret = ft_scs_sync_read_feedback(ids, count, feedbacks, feedback_cap);
    ...
    for(uint8_t i = 0u; i < count; i++) {
        if(ids[i] == ARM_REVERSED_SERVO_ID) {
            float physical_position = feedbacks[i].position;
            float physical_speed = feedbacks[i].speed;
            float logical_position = reversed_servo_physical_to_logical(physical_position);
            float logical_speed = -physical_speed;

            feedbacks[i].position = logical_position;
            feedbacks[i].speed = logical_speed;

            if((uint32_t)(HAL_GetTick() - last_log_ms) >= 200u) {
                last_log_ms = HAL_GetTick();
                log_info("ARM ID2 RX physical=%.4f rad (%.1f deg), logical=%.4f rad (%.1f deg), physical_speed=%.4f, logical_speed=%.4f rad/s",
                         physical_position,
                         RAD_TO_DEG(physical_position),
                         logical_position,
                         RAD_TO_DEG(logical_position),
                         physical_speed,
                         logical_speed);
            }
        }
