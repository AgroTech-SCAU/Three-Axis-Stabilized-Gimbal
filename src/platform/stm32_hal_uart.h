...
/**
 * @brief UART7 句柄
 * @details 用于云台相机（气泡水平仪视觉）接收，115200 8N1（原机械臂串口已让位）
 */
...
/**
 * @brief 将完整日志复制到 USART1 中断发送队列，调用返回后数据已复制
 * @return true 表示成功入队，false 表示参数非法或队列空间不足
 */
...
/**
 * @brief 查询 UART 是否已有中断接收完成回调注册，用于避免阻塞/中断接收争用同一外设
 */
